...
class mpc_config:
    ...
    Qk: list = field(
        default_factory=lambda: np.diag([50, 50, 1.0, 50])
    )  # state error cost matrix, for the the next (T) prediction time steps [x, y, delta, v, yaw, yaw-rate, beta]
    Qfk: list = field(
        default_factory=lambda: np.diag([100, 100, 1.0, 10])
...
class MPC(Node):
    ...
    def pose_callback(self, pose_msg:Odometry):
        ...
        vehicle_state = State(x=vehicle_position[0], y=vehicle_position[1], v=vehicle_velocity, yaw=vehicle_orientation)
        ...
        ref_x, ref_y, ref_yaw, ref_v = self.waypoints[:, 0], self.waypoints[:, 1], self.waypoints[:, 3], self.waypoints[:, 2]
        ...
        # self.publish_testpoints(np.hstack((ox, oy)).reshape(2, -1).T)
    ...
    def mpc_prob_init(self):
        ...
        self.ref_traj_k = cvxpy.Parameter((self.config.NXK, self.config.TK + 1)) # (4, TIME_HORIZON+1)
        ...
        uk_flat = self.uk.flatten(order='F')
        objective += cvxpy.quad_form(uk_flat, R_block)
        # TODO: Objective part 2: Deviation of the vehicle
        # from the reference trajectory weighted by Q,0.006661561773611 including final Timestep T weighted by Qf
        xk_flat = self.xk.flatten(order='F')
        ref_traj_k_flat = self.ref_traj_k.flatten(order='F')
        objective += cvxpy.quad_form(xk_flat - ref_traj_k_flat, Q_block)
        # TODO: Objective part 3: Difference from one control input to the next control input weighted by Rd
        uk_diff = cvxpy.diff(self.uk, axis=1) # ORDER?
        uk_diff = uk_diff.flatten(order='F')
        objective += cvxpy.quad_form(uk_diff, Rd_block)
        # --------------------------------------------------------
        ...
        # print(self.Ak_.shape)
        # print(self.xk[:, :-1].shape)
        # print(self.Bk_ @ self.uk)
        # print(self.Ak_ @ self.xk[:, :-1] + self.Bk_ @ self.uk + self.Ck_)
        constraints.append(self.xk[:, 1:].flatten() == self.Ak_ @ self.xk[:, :-1].flatten() + self.Bk_ @ self.uk.flatten() + self.Ck_)
        ...
        bound = np.ones(self.uk[1, 1:].shape) * (self.config.MAX_DSTEER * self.config.DTK)
        constraints.append(cvxpy.abs(self.uk[1, 1:] - self.uk[1,:-1]) <= bound) ##?
        ...
        # x: [x, y, v, yaw]
        constraints.append(self.xk[:, 0] == self.x0k) # INITIAL STATE CONSTRAINT
        
        constraints.append(self.xk[2, :] <= self.config.MAX_SPEED)
        constraints.append(self.xk[2, :] >= self.config.MIN_SPEED)
        
        constraints.append(self.uk[0, :] <= self.config.MAX_ACCEL) # ACC CONSTRIAN
        constraints.append(self.uk[0, :] >= -self.config.MAX_ACCEL)
        
        constraints.append(self.uk[1, :] <= self.config.MAX_STEER) ##? HOW TO CONSTRIAN STEERING ANGLE
    ...
    def calc_ref_trajectory(self, state:State, cx, cy, cyaw, sp):
        ...
        # self.publish_goalpoint([cx[ind], cy[ind]])
    ...
    def publish_refpoints(self, testpoints):
        markerArray = MarkerArray()
        for i, tp in enumerate(testpoints):
            marker = Marker()
            marker.header.frame_id = "map"
            ...
            marker.color.b = 1.0
            markerArray.markers.append(marker)
        self.refpoint_publisher.publish(markerArray)
